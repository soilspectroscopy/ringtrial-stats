...
p.plsr.vert <- perf.plsr %>%
  ...
  scale_y_continuous(limits = c(-0.3, 1.3), breaks = c(0,0.25,0.50,0.75,1.00)) +
  labs(tittle = "",
       y = "Lin's CCC", x = NULL) +
  theme_light() +
...
sample1 = rnorm(n = 100, mean = 5, sd = 2)
sample2 = rnorm(n = 100, mean = 12, sd = 8)
permutation.test(sample1 = sample1, sample2 = sample2)

sample1 = rnorm(n = 100, mean = 28, sd = 5)
sample2 = rnorm(n = 100, mean = 28, sd = 6)
permutation.test(sample1 = sample1, sample2 = sample2)
...
for(i in 1:length(soil.properties)) {
  ...
  for(k in 1:nrow(prep.spectra.pairs)) {
    ...
    p.value <- permutation.test(sample1 = subset1,
                                sample2 = subset2,
                                p.position = 0.10)
  ...
  permutation.lower.list[[i]] <- reference.grid %>%
    mutate(soil_property = isoil.property, .before = 1)
...
# long format
permutation.lower <- Reduce(bind_rows, permutation.lower.list)
permutation.lower
...
# lower
comparison.lower <- permutation.lower %>%
  mutate(comparison = paste(level1, level2, sep = "-")) %>%
  as.data.frame()

plot.labels.lower <- comparison.lower %>%
  nest_by(soil_property) %>%
  mutate(result = list(cldList(p_value ~ comparison, data = data,
                               threshold = 0.05,
                               reversed = T))) %>%
...
p.plsr.vert.cld <- p.plsr.vert +
  geom_text(data = plot.labels.median, aes(label = letter),
            color = "gray30", size = 3) +
  geom_text(data = plot.labels.lower, aes(label = letter),
            color = "gray30", size = 3) +
  ggtitle("PLSR comparison"); p.plsr.vert.cld

ggsave("outputs/plot_cld_plsr.png", p.plsr.vert.cld,
